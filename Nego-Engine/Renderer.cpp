...
void Renderer::Init(HINSTANCE hInstance, int width, int height)
{
	...
	hWnd = CreateWindow(WindowName, WindowName, WS_CAPTION | WS_POPUPWINDOW | WS_VISIBLE, 0, 0, CurrentScreenSize.X, CurrentScreenSize.Y, NULL, NULL, hInstance, NULL);
	hDC = GetDC(hWnd);
...
void Renderer::PreRenderSetup()
{
	...
	gluPerspective(45.0f, CurrentScreenSize.X / (double)CurrentScreenSize.Y, 0.1, 5000);
